...
export default function InstaShowcase() {
  ...
  // Scroll edges — used to enable/disable the desktop prev/next arrows
  ...
  const scrollBy = (dir) => {
    const el = railRef.current
    if (!el) return
    const first = el.querySelector('.insta-card')
  ...
  return (
    <section className="insta">
      <div className="insta__container">
        ...
        <div className="insta__rail-wrap">
          <button
            type="button"
            className="insta__arrow insta__arrow--prev"
            onClick={() => scrollBy(-1)}
            disabled={atStart}
          ...
          <div className="insta__grid" ref={railRef}>
            {items.map((it) => (
              <InstaCard
                key={it._id}
                post={it}
          ...
          <button
            type="button"
            className="insta__arrow insta__arrow--next"
            onClick={() => scrollBy(1)}
            disabled={atEnd}
